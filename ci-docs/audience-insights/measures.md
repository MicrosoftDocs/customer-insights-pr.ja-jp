...
1. **名前の編集** を選択して、メジャーの **名前** を提供します。 
   > [!NOTE]
   > 新しいメジャー構成に 2 つのフィールド — たとえば、CustomerID と 1 つの計算 — しかない場合、出力は、Customer_Measure と呼ばれるシステム生成エンティティに新しい列として追加されます。 また、統一された顧客プロファイルでメジャーの値を確認できます。 他のメジャーは、独自のエンティティを生成します。
...
1. データの中に整数に置き換える必要のある値がある場合 — 例: *null* を *0* に置き換える — **ルール** を選択します。 ルールを構成し、置換として整数のみを選択していることを確認します。
