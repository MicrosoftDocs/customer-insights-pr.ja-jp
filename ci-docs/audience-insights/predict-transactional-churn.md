...
ms.locfileid: "7643417"
---
# <a name="transaction-churn-prediction-preview"></a>トランザクション離反の予測 (プレビュー)
...
> [!TIP]
> サンプル データを使用して、トランザクション離反予測のチュートリアルを試してください。[トランザクション離反予測 (プレビュー) サンプル ガイド](sample-guide-predict-transactional-churn.md)。
...
- (オプション) 顧客に関するデータ:
    - このデータは、モデルが最高のパフォーマンスを発揮するように、より静的な属性に向けて調整する必要があります。
...
1. セマンティックの属性を、モデルの実行に必要なフィールドにマッピングします。 以下のフィールドが入力されていない場合は、購入履歴エンティティから *顧客* エンティティへの関係を構成します。
...
1. セカンダリ レベルとして使用する属性を選択してから、**追加** を選択します。
...
### <a name="provide-an-optional-list-of-benchmark-accounts-business-accounts-only"></a>ベンチマーク アカウントのオプションのリストを提供します (ビジネスアカウントのみ)
